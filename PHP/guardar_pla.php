...
$id_varietat = !empty($_POST['id_varietat']) ? $_POST['id_varietat'] : NULL;
...
$sql = $conn->prepare("INSERT INTO pla_tractament 
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
$sql->bind_param("ssiiiisss", $nom, $tipus, $id_estat_inici, $id_estat_fi, $id_varietat, $finestra_data_inici, $finestra_data_fi, $plaga_malaltia_objectiu, $observacions);

if ($sql->execute()) {
    echo "<h3>Pla de tractament guardat correctament!</h3>";
    echo "<a href='../HTML/pla_tractament.html'>Tornar</a>";
} else {
